        ...
        <div className='bg-white rounded shadow p-2 flex gap-1 w-full h-[75vh] '>
            ...
            <div className='flex flex-col w-1/4  h-full  '>
                <div className='p-2'>
                    ...
                    <p>Total Conversation: {conversations.length}</p>
                </div>
                <div className='overflow-y-scroll h-full border border-neutral-200'>
                    {conversations.map((conversation) => (
                        <div
                            ...
                            <div className='flex-1'>
                                <h1 className='text-sm '>Guest</h1>
                                <p className='text-xs text-neutral-500'>{conversation.lastMessage?.substring(0, 50)}</p>
                ...
                {status === "succeeded" && chats.length > 0 && (
                    <div className='flex flex-col gap-2 h-full'>
                        <div className='bg-white sticky top-0 w-full z-10'>
                            <h1 className='text-lg font-semibold'>Chat with Guest</h1>
                            <p>{chats[0].recipientID}</p>
                        </div>
                        <div className='flex-1 overflow-y-scroll flex flex-col gap-2 p-2 relative bg-slate-100'>

                            {chats.map((chat) => (
                                <>
                                    {/* <div>
                    <h1>{chat.senderID === '69843421d30a0ace506d9172' ? 'You' : 'Guest'}</h1>
                  </div> */}
                                    <div key={chat._id} className={`p-2 min-w-54 rounded  ${chat.senderID === '69843421d30a0ace506d9172' ? 'bg-green-100 self-end border border-green-200' : 'bg-white border border-neutral-200  self-start'}`}>
                                        ...
                                        <div className='text-neutral-400 text-xs flex items-center gap-1 justify-end'>
                                            ...
                                            {chat.senderID === '69843421d30a0ace506d9172'  && 
                                            <>
                                            {chat.isread === undefined ? <Check size={15} className='inline-block text-gray-400' /> : <> {chat.isread ? <CheckCheck size={15} className='inline-block text-blue-500' /> : <CheckCheck size={15} className='inline-block text-gray-400' />}</>}
                                            </>
                                            }
